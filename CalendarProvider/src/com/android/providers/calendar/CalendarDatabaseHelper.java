...
/* package */ class CalendarDatabaseHelper extends SQLiteOpenHelper {
    ...
    private static final String LAST_SYNCED_EVENT_COLUMNS =
            ...
            Events.TAG_ID + "," +
            Events.SLEEP_SLIENT + "," +
            Events.REMINDER_RES_UPDATE + "," +
            Events.REMINDER_GAME_RESULT + "," +
            Events.REMINDER_TV_PLAY + "," +
            Events.TYPE + "," +
            Events.CONTACT_ID + "," +
            Events.TAG_EVENT_ID + "," +
            Events.UPDATE_SYNC_ID;
    ...
    private DatabaseUtils.InsertHelper mTagsInserter;
    private DatabaseUtils.InsertHelper mTagSettingsInserter;
    ...
    public long tagsInsert(ContentValues values) {
        return mTagsInserter.insert(values);
    }

    public long tagSettingsInsert(ContentValues values) {
        return mTagSettingsInserter.insert(values);
    }
    ...
    public void onOpen(SQLiteDatabase db) {
        ...
        mTagsInserter = new DatabaseUtils.InsertHelper(db, Tables.TAGS);
        mTagSettingsInserter = new DatabaseUtils.InsertHelper(db, Tables.TAG_SETTINGS);
    }
    ...
    private void createTagSettingsTable(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE IF NOT EXISTS "+ Tables.TAG_SETTINGS + "(" +
                CalendarContract.TagsSettings.TAG_ID + " INTEGER PRIMARY KEY," +
                CalendarContract.TagsSettings.NEED_REMINDER + " INTEGER," +
                CalendarContract.TagsSettings.SLEEP_SLIENT + " INTEGER," +
                CalendarContract.TagsSettings.RES_UPDATE_REMIND + " INTEGER," +
                CalendarContract.TagsSettings.GAME_RESULT_REMIND + " INTEGER," +
                CalendarContract.TagsSettings.REMINDER_TIME + " INTEGER)");
    }

    private void createTagsTable(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE IF NOT EXISTS " + Tables.TAGS + "(" +
                CalendarContract.Tags._ID + " INTEGER PRIMARY KEY AUTOINCREMENT," +
                CalendarContract.Tags.TAG_ID + " INTEGER," +
                CalendarContract.Tags.NAME + " TEXT," +
                CalendarContract.Tags.CATEGORIES + " TEXT," +
                CalendarContract.Tags.IMAGE +" TEXT," +
                CalendarContract.Tags.ICON +" TEXT," +
                CalendarContract.Tags.TYPE + " INTEGER)");
    }
    ...
    private void createEventsTable(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE " + Tables.EVENTS + " (" +
               ...
               ///M:#extension# #PC Sync# @{
                ...
                ///@}
                CalendarContract.Events.TAG_ID + " TEXT," +
                CalendarContract.Events.SLEEP_SLIENT + " INTEGER," +
                CalendarContract.Events.REMINDER_RES_UPDATE + " INTEGER," +
                CalendarContract.Events.REMINDER_GAME_RESULT + " INTEGER," +
                CalendarContract.Events.REMINDER_TV_PLAY + " INTEGER," +
                CalendarContract.Events.TYPE + " INTEGER," +
                CalendarContract.Events.CONTACT_ID + " INTEGER," +
                CalendarContract.Events.TAG_EVENT_ID + " INTEGER," +
//                CalendarContract.Events.UPDATE_SYNC_ID + " TEXT);");
                // [+LEUI] [REQ][SI-17731] [huangzhihao] add 2015.7.7 function: upgrade to 606
                CalendarContract.Events.UPDATE_SYNC_ID + " TEXT," +
                "UNIQUE (" + CalendarContract.Events.TAG_EVENT_ID + ")"  +
                ");");
                // [-LEUI]
    ...
    /**********************************************************/
    /**
     * [+LEUI] [REQ][SI-17731] [huangzhihao] add 2015.7.7
     * function: upgrade to 606
     * @hide
     */
    private void upgradeToVersion606(SQLiteDatabase db) {
        ...
        String FIELD_LIST =
                ...
                "tag_event_id, " +
                "update_sync_id ";
        // copy fields from old to new, its some tricky that we copy data with not-null tag_event_id, 
        //and then copy data with null tag_event_id. so group-by will not lose data with null tag_event_id.
        //by @huangzhihao
        db.execSQL("INSERT INTO Events (" + FIELD_LIST + ") SELECT " + FIELD_LIST +
                "FROM Events_Backup WHERE tag_event_id NOT NULL GROUP BY tag_event_id;");
        db.execSQL("INSERT INTO Events (" + FIELD_LIST + ") SELECT " + FIELD_LIST +
                "FROM Events_Backup WHERE tag_event_id IS NULL;");
        ...
        // Triggers to update the color stored in an event when
        // the color_index is changed.
        db.execSQL(CREATE_EVENT_COLOR_UPDATE_TRIGGER);
    ...
    private void upgradeToVersion607(SQLiteDatabase db){
        ...
        String FIELD_606_LIST =
                        ...
                        "update_sync_id, " +
                        "sleepSlient, " +         //59
                        "reminder_resUpdate, " +  //60
                        "reminder_gameResult, " + //61
                        "reminder_TVPlay" ;       //62

        String FIELD_607_LIST =
                "_id, " +
                        "_sync_id, " +
                        "dirty, " +
                        "mutators, " +
        ...
        Cursor cursor=db.query("Events_Backup",FIELD_606_LIST.split(",") , null,
                null, null, null, null);

        reinsertIntoEventfor607(cursor, db);
    ...
    private void reinsertIntoEventfor607(Cursor cursor,SQLiteDatabase db){
                ...
                while (cursor.moveToNext()){
                    ...
                    for (int i=0;i<59;i++){
                        if (TextUtils.isEmpty(cursor.getString(i))){
                            selection.append(cursor.getColumnName(i)).append(",");
                            values.append(cursor.getString(i)).append(",");
                        }
                    }
                    ...
                    }else if (cursor.getColumnCount()==59){
                        if (cursor.getInt(59)==1){
                            type+=4;
                        }
                    }else if (cursor.getColumnCount()==59){
                    ...
                    values.append(type).append(")");
                    db.execSQL("INSERT INTO Events "+selection +" values "+values);
                }
    ...
    private void upgradeToVersion601(SQLiteDatabase db) {
        /*
         * Changes from version 600 to 601:
         * - add tag_id, sleepSlient, reminder_resUpdate, reminder_result, reminder_TVPlay columns to Events
         */
        db.execSQL("ALTER TABLE Events ADD COLUMN "+ CalendarContract.Events.TAG_ID +" TEXT;");
        db.execSQL("ALTER TABLE Events ADD COLUMN "+ CalendarContract.Events.SLEEP_SLIENT +" INTEGER;");
        db.execSQL("ALTER TABLE Events ADD COLUMN "+ CalendarContract.Events.REMINDER_RES_UPDATE +" INTEGER;");
        db.execSQL("ALTER TABLE Events ADD COLUMN "+ CalendarContract.Events.REMINDER_GAME_RESULT +" INTEGER;");
        db.execSQL("ALTER TABLE Events ADD COLUMN "+ CalendarContract.Events.REMINDER_TV_PLAY +" INTEGER;");
    }
    private void upgradeToVersion602(SQLiteDatabase db) {
        /*
         * Changes from version 600 to 602:
         * - add tag_event_id column to Events
         */
        db.execSQL("ALTER TABLE Events ADD COLUMN "+ CalendarContract.Events.TAG_EVENT_ID +" INTEGER;");
    }

    private void upgradeToVersion605(SQLiteDatabase db) {
        /*
         * Changes from version 604 to 605:
         * - add update_sync_id column to Events
         */
        db.execSQL("ALTER TABLE Events ADD COLUMN "+ CalendarContract.Events.UPDATE_SYNC_ID +" TEXT;");
    }
    ...
    private static void createEventsView(SQLiteDatabase db) {
        ...
        String eventsSelect = "SELECT "
                ...
                + ExtConsts.LunarEvent.LUNAR_RRULE + ","
                /// @}
                + CalendarContract.Events.TAG_ID + ","
                + CalendarContract.Events.SLEEP_SLIENT + ","
                + CalendarContract.Events.REMINDER_RES_UPDATE + ","
                + CalendarContract.Events.REMINDER_GAME_RESULT + ","
                + CalendarContract.Events.REMINDER_TV_PLAY + ","
                + CalendarContract.Events.TYPE + ","
                + CalendarContract.Events.CONTACT_ID + ","
                + CalendarContract.Events.TAG_EVENT_ID + ","
                + CalendarContract.Events.UPDATE_SYNC_ID + ","
                ...
                + "=" + Tables.CALENDARS + "." + Calendars._ID + " OR (" + Tables.EVENTS + "." + Events.TAG_ID + " IS NOT NULL AND ("
                + Tables.CALENDARS + "." + Calendars.ACCOUNT_TYPE + "=\"com.letv\" OR " + Tables.CALENDARS + "." + Calendars.ACCOUNT_NAME + "=\"letv@local\"))"
                + ")";
