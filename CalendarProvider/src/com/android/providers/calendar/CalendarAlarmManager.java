...
public class CalendarAlarmManager {
    ...
    /**
     * M: Modify alarm check delay millisecond from 5000ms to 3000ms to make
     * sure the broadcast android.intent.action.EVENT_REMINDER is sent before
     * the broadcast android.intent.action.PROVIDER_CHANGED to avoid CTS time
     * out.
     *
     * @{
     */
    static final int ALARM_CHECK_DELAY_MILLIS = 3000;
    /** @}*/
    ...
    void scheduleNextAlarm(boolean removeAlarms) {
        ...
        if (!mNextAlarmCheckScheduled.getAndSet(true) || removeAlarms) {
            ...
            /** M: add some log to record schedule time. @{*/
            Log.i(CalendarProvider2.TAG, "scheduleNextAlarm after "
                    + ALARM_CHECK_DELAY_MILLIS + "ms" + ", pending=" + pending);
            /** @} */
        }
    }
