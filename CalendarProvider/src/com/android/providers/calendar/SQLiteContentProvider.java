...
public abstract class SQLiteContentProvider extends ContentProvider
    ...
    protected abstract void notifyChange(boolean syncToNetwork, Uri...uris);
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        if (!applyingBatch) {
            ...
            onEndTransaction(!isCallerSyncAdapter && shouldSyncFor(uri), result);
        } else {
            result = insertInTransaction(uri, values, isCallerSyncAdapter);
            if (result != null) {
                mNotifyChange = true;
    ...
    public int bulkInsert(Uri uri, ContentValues[] values) {
        ...
        onEndTransaction(!isCallerSyncAdapter, uris);
        return numValues;
    }
    ...
    public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        if (!applyingBatch) {
            ...
            onEndTransaction(!isCallerSyncAdapter && shouldSyncFor(uri), uri);
        } else {
            count = updateInTransaction(uri, values, selection, selectionArgs,
                        isCallerSyncAdapter);
            if (count > 0) {
    ...
    public ContentProviderResult[] applyBatch(ArrayList<ContentProviderOperation> operations)
        ...
        try {
            ...
            for (int i = 0; i < numOperations; i++) {
                ...
                results[i] = operation.apply(this, results, i, uris);
            }
            mDb.setTransactionSuccessful();
            return results;
        } finally {
            ...
            onEndTransaction(!isCallerSyncAdapter, uriArr);
            Binder.restoreCallingIdentity(identity);
        }
    }
    ...
    protected void onEndTransaction(boolean syncToNetwork, Uri...uris) {
        if (mNotifyChange) {
            mNotifyChange = false;
            // We sync to network if the caller was not the sync adapter
            notifyChange(syncToNetwork, uris);
        }
    }
